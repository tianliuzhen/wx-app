...
Page({
  ...
  onPullDownRefresh: function () {
    ...
    this.openDoorAfter(); // this.getQrocdeByClick(wx.getStorageSync("userInfoVisitor").openId)
    ...
    wx.login({
      success: res => {
        ...
        request({
          ...
          socket.closeSocket(res.data.data); // 连接socket
  ...
  checkUser(userInfo) {
    ...
    if (userInfo != null && userInfo != "") {
      ...
      if (userInfo.status == 1) {
        ...
        //获取当前时间
        var currentTime = new Date(); //自定义时间
  ...
  getQrocdeByClick(openId) {
    request({
      ...
      if (res.data.success) {
        this.setData({
          qrCode: res.data.data // isNewUser: false,
          // isShowQrCode: true,
