...
Page({
  ...
  data: {
    ...
    // 系统默认亮度次数
    //小区-普通选择器：（普通数组）
    areaList: [{"id":"1265476890672672773","name":"建华家苑小区","code":"hyznkj_jhjy","checked":null},{"id":"1265476890672672774","name":"北城天地小区","code":"hyznkj_bctd","checked":null},{"id":"1430706197469945858","name":"ZDM","code":"CDZDM","checked":null},{"id":"1431413986446106625","name":"JKDX","code":"JKDX","checked":null}],
    //远程开门-普通选择器：（普通数组）
    deviceList: [],
    areaListIndex: "",
    // 选择框选中值
  ...
  onLoad: function (options) {
    console.log("onLoad"); // 一般这里发送页面请求初始化页面
    // 初始化用户信息
    // this.initUser()
  },
  onShareAppMessage: function () {
    ...
    return {
      title: "千里码门禁",
      path: "/pages/biz/registerUser/registerUser" //这是一个路径
  ...
  onShow: function () {
    ...
    // todo
      socket.openSocket({"openId":"123456",type:"0"}, this);
  ...
  onHide: function () {
    ...
    blueTooth.closeBlueTooth(this); // 关闭socket

      // todo
      socket.closeSocket({"openId":"123456",type:"0"});
  ...
  refushSocket() {
    ...
    if (wx.getStorageSync("userInfo") == null || wx.getStorageSync("userInfo") == "") {
        ...
        success: res => {
          request({
            ...
            if (res.data.data != null) {
              wx.setStorageSync("userInfo", res.data.data); // 连接socket
      ...
      socket.closeSocket(wx.getStorageSync("userInfo")); // 连接socket
  ...
  onPullDownRefresh: function () {
    ...
    this.openDoorAfter(); // this.getQrocdeByClick(wx.getStorageSync("userInfo").openId)
    // 当处理完数据刷新后，wx.stopPullDownRefresh可以停止当前页面的下拉刷新。

    wx.stopPullDownRefresh(); // 必须先关
    // socket.closeSocket(wx.getStorageSync("userInfo"))
    // socket 连接处理
    ...
    wx.login({
      success: res => {
        ...
        request({
          ...
          this.checkUser(res.data.data); // 初始化小区设备列表

          this.initDataCheckObj(); // 如果是正常的用户
          // console.log(res.data.data);
  ...
  checkUser(userInfo) {
    ...
    if (userInfo != null && userInfo != "") {
      ...
      if (userInfo.status == 1) {
        ...
        this.getQrocdeByClick(userInfo.openId); // 审批中
  ...
  formSubmit: function (e) {
    ...
    let {
      ...
      doorNo,
      floorNo,
      buildingNo // verificationCode

    } = e.detail.value; // 待确定1：是否需要短信验证 （|| !verificationCode ）
  ...
  reLoadImage: function (event) {// console.log(event)
    // this.setData({
    //   qrCode:""
    // })
  },
  ...
  confirm: function () {
    this.closeDialog(); // this._toast('confirm')

    this.checkForChecked();
  },
  cancel: function () {
    this.closeDialog(); // this._toast('cancel')
  },
  ...
  confirmBlue() {
    blueTooth.sendBLECharacterNotice(this); // blueTooth.closeBlueTooth(this)
