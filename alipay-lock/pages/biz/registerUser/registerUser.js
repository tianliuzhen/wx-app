...
Page({
    ...
    refushSocket() {
        ...
        if (wx.getStorageSync("userInfo") == null || wx.getStorageSync("userInfo") == "") {
            wx.login({
                success: res => {
                    request({
                        url: app.globalData.api_getUserInfo + "?types=0,1&sdkType=alipay",
                        method: "get",
                        data: {
                            jsCode: res.code
                        }
    ...
    checkUser(userInfo) {
        ...
        if (userInfo != null && userInfo != "") {
            ...
            if (userInfo.status == 1) {
                ...
                this.getQrocdeByClick(userInfo.userId);
                // 审批中
    ...
    getQrocdeByClick(openId) {
        request({
            url: app.globalData.api_getQrCode + "?openId=" + openId + "&types=0,1&sdkType=alipay",
            method: "post"
        }).then(res => {
            if (res.data.success) {
                this.setData({
    ...
    setTime() {
        ...
        interval = setInterval(function () {
            ...
            if (that.data.time <= 0) {
                clearInterval(interval);
                that.getQrocdeByClick(wx.getStorageSync("userInfo").userId);
                that.setData({
                    time: 10 * 6
                });
            }
