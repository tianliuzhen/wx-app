...
var thisGlobal = null; // var sign= "Feas"
...
function initBlueTooth(pointer) {
    ...
    wx.getBluetoothDevices({
        success: function(res) {
            console.log("发现设备:", res); //5s内未搜索到设备，关闭搜索，关闭蓝牙模块
            ...
            setTimeout(function() {
                if (!that.data.deviceId) {
                    ...
                    wx.stopBluetoothDevicesDiscovery(); //关闭蓝牙
    ...
    wx.onBluetoothDeviceFound(function(devices) {
        ...
        for (let i = 0; i < devices.devices.length; i++) {
            ...
            if (devices.devices[i].name.substr(0, 4) == sign) {
                ...
                toConnectionBlueToothDevice(); //关闭搜索
    ...
    wx.getBLEDeviceCharacteristics({
        ...
        success: function(res) {
            ...
            for (let i = 0; i < res.characteristics.length; i++) {
                let charc = res.characteristics[i]; // if (charc.properties.indicate) {
                //   that.setData({
                ...
                if (charc.properties.write) {
                    ...
                    console.log("写write_id:", that.data.write_id); // todo 自动执行，转为手动执行
...
function recvBLECharacterNotice(deviceId, serviceId, charId) {
    wx.notifyBLECharacteristicValueChange({
        ...
        //服务特征值indicate
        success: function(res) {
            console.log("开启notify", res.errMsg); //监听低功耗蓝牙设备的特征值变化
...
function sendBLECharacterNotice(pointer) {
    thisGlobal = pointer;
    var that = thisGlobal; //写入数据

    request({
        url:
            app.globalData.api_getQrCodeDataByBluetooth +
            "?openId=" +
...
function wrireToBlueToothDevice(msg) {
    ...
    while (bytes > 0) {
        ...
        if (bytes > 20) {
            ...
            pos += 20;
            bytes -= 20; // var that = thisGlobal
            // toWriteBLECharacteristicValue(that,tmpBuffer)
        } else {
            tmpBuffer = buffer.slice(pos, pos + bytes);
            dataList.push(tmpBuffer); // tmpBuffer = buffer.substr(pos, pos + 20)

            console.log("pos: " + pos + " pos2: " + (pos + bytes));
            pos += bytes;
            bytes -= bytes; // var that = thisGlobal
            // toWriteBLECharacteristicValue(that,tmpBuffer)
        }
    }
...
function toWriteBLECharacteristicValueLoop() {
    ...
    if (init < dataList.length) {
        ...
        wx.writeBLECharacteristicValue({
            ...
            fail: function(res) {
                console.log("发送失败", res); // 发送失败递归发送直到发送成功为止
                // toWriteBLECharacteristicValue(that,tmpBuffer)
            }
        });
    } else {
